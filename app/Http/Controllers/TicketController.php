...
class TicketController extends Controller
{
    ...
    // ← ADDED: Show a single ticket
    public function show(Ticket $ticket)
    {
        // Only allow the ticket owner to view
        if ($ticket->booking->user_id !== auth()->id()) {
            abort(403);
        }

        $ticket->load(['booking.schedule.trip', 'booking.user']);
    ...
    public function cancel(Ticket $ticket)
    {
        ...
        // Update ticket status only — do NOT cancel the entire booking
        $ticket->update(['status' => 'cancelled']);

        // Restore only 1 seat for this individual ticket
        if ($ticket->booking->schedule) {
            $ticket->booking->schedule->increment('available_seats', 1);
        }
