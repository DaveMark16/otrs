...
<div class="route-bar">
  ...
  <div class="route-middle">
    ...
    <div style="font-size:.72rem;color:rgba(59,42,26,.35);">Direct · {{ $trip->operator ?? 'N/A' }}</div>
    <div class="fstats">
      <div><div class="fstat-label">Hours</div><div class="fstat-val">{{ $hours }}h</div></div>
      <div><div class="fstat-label">Minutes</div><div class="fstat-val">{{ $remMins }}m</div></div>
...
<div class="detail-grid">
  ...
  <div class="card">
    ...
    <div class="card-section">
      ...
      <div class="info-row"><span class="info-k">Airline</span><span class="info-v">{{ $trip->operator ?? '—' }}</span></div>
      <div class="info-row"><span class="info-k">Class</span><span class="info-v">{{ ucfirst($schedule->fare_class ?? 'Economy') }}</span></div>
      <div class="info-row"><span class="info-k">Passengers</span><span class="info-v">{{ $booking->passenger_count }} adult(s)</span></div>
      <div class="info-row"><span class="info-k">Booked</span><span class="info-v">{{ $booking->created_at->format('M d, Y') }}</span></div>
    </div>
  </div>
  ...
  <div>
    <div class="card" style="margin-bottom:14px;">
      <div class="card-section">
        ...
        <div class="amount-box">
          @if($booking->has_promo ?? false)
            <div style="font-size:.76rem;color:rgba(59,42,26,.35);text-decoration:line-through;margin-bottom:2px;">₱{{ number_format($booking->original_amount ?? 0, 2) }}</div>
          @endif
          <div class="amount-main">₱{{ number_format($booking->total_amount, 2) }}</div>
          <div class="amount-sub">inclusive of taxes &amp; fees</div>
          @if($booking->has_promo ?? false)
            <div class="promo-tag">🏷 {{ $booking->promo->promo_code ?? '' }} — saved ₱{{ number_format($booking->discount_amount ?? 0, 2) }}</div>
          @endif
        </div>
      </div>
      ...
      <div class="card-section">
        ...
        <div class="info-row"><span class="info-k">Ref</span><span class="info-v" style="font-size:.72rem;font-family:monospace;">{{ $booking->payment->transaction_ref ?? '—' }}</span></div>
      </div>
      @endif
      ...
      <div class="card-section">
        ...
        @elseif($s === 'confirmed' && !$booking->payment)
          ...
          <div style="display:flex;gap:10px;flex-wrap:wrap;">
            <form method="POST" action="{{ route('bookings.pay', $booking->id) }}" style="display:inline"
                  onsubmit="return confirm('Pay ₱{{ number_format($booking->total_amount,2) }}?')">
              @csrf
...
document.addEventListener('DOMContentLoaded', function () {
  var oLat={{ $originCoords[0] }},oLng={{ $originCoords[1] }},dLat={{ $destCoords[0] }},dLng={{ $destCoords[1] }};
