...
<div class="stats-grid" style="margin-bottom:14px;">
  <div class="stat-card" style="--ac:var(--teal)">
    ...
    <div class="stat-sub">{{ $stats['admins'] ?? 0 }} admins</div>
  </div>
  ...
  <div class="stat-card" style="--ac:var(--teal)">
    ...
    <div class="stat-value" style="font-size:1.5rem;">&#8369;{{ number_format($stats['total_revenue'], 0) }}</div>
...
<div class="stats-grid" style="margin-bottom:26px;">
  ...
  <div class="stat-card" style="--ac:var(--teal)">
    ...
    <div class="stat-value" style="font-size:1.5rem;">&#8369;{{ number_format($stats['paid_payments'], 0) }}</div>
...
<div class="row2">
  ...
  <div class="panel">
    <div class="panel-head">
      ...
      <a href="{{ route('admin.bookings.index') }}" class="panel-link">View all →</a>
    </div>
    ...
    <table class="mini-table">
      ...
      <tbody>
        ...
        <tr>
          <td><span class="ref-badge">{{ $b->reference_no }}</span></td>
          <td>
            <div class="user-chip">
              <div class="user-av">{{ strtoupper(substr($b->user->name ?? 'U', 0, 2)) }}</div>
              <span style="font-weight:600;color:var(--brown);font-size:.83rem;">{{ $b->user->name ?? '—' }}</span>
            </div>
          </td>
          <td>{{ $b->schedule->trip->origin ?? '?' }} → {{ $b->schedule->trip->destination ?? '?' }}</td>
          <td>
            @php $bs = $b->status; @endphp
            <span class="pill {{ $bs==='confirmed'?'pill-green':($bs==='pending'?'pill-amber':($bs==='cancelled'?'pill-red':($bs==='ticketed'?'pill-blue':'pill-gray'))) }}">
              {{ ucfirst($bs) }}
            </span>
          </td>
          <td class="td-amount">&#8369;{{ number_format($b->total_amount, 2) }}</td>
          <td>
            <a href="{{ route('admin.bookings.show', $b) }}" class="view-link">
  ...
  <div>
    ...
    <div class="panel">
      ...
      <div class="quick-grid">
        <a href="{{ route('admin.bookings.index', ['status'=>'pending']) }}" class="quick-btn">
        ...
        <a href="{{ route('admin.trips.create') }}" class="quick-btn">
          <div class="quick-icon" style="color:var(--tan)">
            <svg viewBox="0 0 24 24"><path d="M17.8 19.2L16 11l3.5-3.5C21 6 21 4 19 4c-1 0-1.5.5-3.5 2.5L11 8.2 2.8 6.4 2 7.2l3 5.5-2 1.7v2.2l3-1.5 1.5 3h2.2l1.7-2 5.5 3z"/></svg>
          </div>
          <span class="quick-label">Add Trip</span>
        </a>
        <a href="{{ route('admin.payments.index') }}" class="quick-btn">
          <div class="quick-icon">
            <svg viewBox="0 0 24 24"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>
          </div>
    ...
    <div class="panel">
      <div class="panel-head">
        ...
        <a href="{{ route('admin.users.index') }}" class="panel-link">View all →</a>
      </div>
      @foreach($recent_users as $u)
      <div class="rur">
        <div class="user-av">{{ strtoupper(substr($u->name, 0, 2)) }}</div>
        <div class="rur-meta">
          <div class="rur-name">{{ $u->name }}</div>
          <div class="rur-email">{{ $u->email }}</div>
        </div>
        <span class="pill {{ $u->role==='admin'||$u->role==='superadmin'?'pill-amber':'pill-gray' }}">{{ ucfirst($u->role) }}</span>
      </div>
      @endforeach
    </div>
  </div>
