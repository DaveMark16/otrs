...
<form method="GET" action="{{ route('admin.schedules.index') }}">
  <div class="filters-bar">
    <input type="text" name="search" placeholder="Search trip, route, operator…" value="{{ request('search') }}">
    ...
    @if(request()->hasAny(['search','fare_class','status']))
      <a href="{{ route('admin.schedules.index') }}" class="clear-link">Clear</a>
    @endif
  </div>
</form>
...
<div class="table-wrap">
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        <td>
          <div class="trip-name">{{ $schedule->trip->name ?? '—' }}</div>
          <div class="trip-sub">
            {{ $schedule->trip->origin ?? '?' }}
            <span class="arr">→</span>
            {{ $schedule->trip->destination ?? '?' }}
            @if($schedule->trip->operator ?? null)
              &nbsp;·&nbsp;{{ $schedule->trip->operator }}
            @endif
          </div>
        </td>
        <td>
          <div style="font-weight:600;color:var(--brown);font-size:.83rem;">{{ $schedule->departure_at->format('M d, Y') }}</div>
          <div style="font-size:.72rem;color:rgba(59,42,26,.38);">{{ $schedule->departure_at->format('H:i') }}</div>
        </td>
        <td>
          <div style="font-weight:600;color:var(--brown);font-size:.83rem;">{{ $schedule->arrival_at->format('M d, Y') }}</div>
          <div style="font-size:.72rem;color:rgba(59,42,26,.38);">{{ $schedule->arrival_at->format('H:i') }}</div>
        </td>
        <td>
          @php $fc = $schedule->fare_class; @endphp
          <span class="pill {{ $fc==='first'?'pill-amber':($fc==='business'?'pill-blue':'pill-gray') }}">
            {{ ucfirst($fc) }}
          </span>
        </td>
        <td style="font-family:monospace;font-weight:700;color:var(--gold);">&#8369;{{ number_format($schedule->base_fare, 2) }}</td>
        <td>
          <span style="font-weight:700;color:{{ $schedule->available_seats < 10 ? '#b44444' : 'var(--teal)' }};">
            {{ $schedule->available_seats }}
          </span>
          <span style="color:rgba(59,42,26,.25);font-size:.78rem;"> / {{ $schedule->capacity }}</span>
        </td>
        <td>
          @php $st = $schedule->status; @endphp
          <span class="pill {{ $st==='scheduled'?'pill-green':($st==='completed'?'pill-blue':'pill-red') }}">
            {{ ucfirst($st) }}
          </span>
        </td>
        <td>
          <div style="display:flex;gap:5px;align-items:center;">
            <a href="{{ route('admin.schedules.edit', $schedule) }}" class="act-btn act-edit">Edit</a>
  ...
  @else
    <div style="padding:52px;text-align:center;color:rgba(59,42,26,.3);font-size:.88rem;">
      No schedules found.
      <a href="{{ route('admin.schedules.create') }}" style="color:var(--teal);text-decoration:none;margin-left:4px;font-weight:600;">Add one →</a>
    </div>
  @endif
</div>

<div class="pager">{{ $schedules->withQueryString()->links() }}</div>
