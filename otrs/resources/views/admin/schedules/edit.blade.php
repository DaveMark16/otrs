...
<div class="sch-header">
  <div>
    <div class="sch-title">Edit Schedule #{{ $schedule->id }}</div>
    <div class="sch-sub">{{ $schedule->trip->name }} — {{ $schedule->trip->origin }} → {{ $schedule->trip->destination }}</div>
  </div>
  ...
  <div class="f-group" style="margin-bottom:16px;">
    ...
    <select name="trip_id" class="f-select" required>
      ...
      @foreach($trips as $trip)
        <option value="{{ $trip->id }}" {{ old('trip_id',$schedule->trip_id)==$trip->id?'selected':'' }}>
  ...
  <div class="f-grid">
    <div class="f-group">
      ...
      <input type="datetime-local" name="departure_at" class="f-input"
             value="{{ old('departure_at', $schedule->departure_at->format('Y-m-d\TH:i')) }}" required>
    </div>
    <div class="f-group">
      <label class="f-label">Arrival Date & Time <span class="req">*</span></label>
      <input type="datetime-local" name="arrival_at" class="f-input"
             value="{{ old('arrival_at', $schedule->arrival_at->format('Y-m-d\TH:i')) }}" required>
    </div>
  </div>
</div>
  ...
  <div class="f-grid">
    <div class="f-group">
      ...
      <input type="number" name="capacity" class="f-input"
             value="{{ old('capacity', $schedule->capacity) }}" min="1" max="1000" required>
    </div>
    <div class="f-group">
      <label class="f-label">Available Seats <span class="req">*</span></label>
      <input type="number" name="available_seats" class="f-input"
             value="{{ old('available_seats', $schedule->available_seats) }}" min="0" required>
    ...
    <div class="f-group">
      ...
      <select name="fare_class" class="f-select" required>
        <option value="economy"  {{ old('fare_class',$schedule->fare_class)==='economy'?'selected':'' }}>Economy</option>
        <option value="business" {{ old('fare_class',$schedule->fare_class)==='business'?'selected':'' }}>Business</option>
        <option value="first"    {{ old('fare_class',$schedule->fare_class)==='first'?'selected':'' }}>First Class</option>
      </select>
    </div>
    <div class="f-group">
      <label class="f-label">Base Fare (PHP) <span class="req">*</span></label>
      <input type="number" name="base_fare" class="f-input"
             value="{{ old('base_fare', $schedule->base_fare) }}" min="0" step="0.01" required>
    </div>
    <div class="f-group">
