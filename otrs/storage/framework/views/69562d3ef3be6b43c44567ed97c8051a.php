...
<div class="stats-row">
  <div class="stat-card" style="--ac:var(--teal)">
    ...
    <div class="stat-val" style="font-size:1.5rem;">&#8369;<?php echo e(number_format($stats['total_paid'],0)); ?></div>
    <div class="stat-sub">Verified payments</div>
  </div>
  <div class="stat-card" style="--ac:#a07830">
    <div class="stat-label">Pending</div>
    <div class="stat-val"><?php echo e($stats['pending']); ?></div>
  ...
  <div class="stat-card" style="--ac:var(--teal)">
    ...
    <div class="stat-val" style="font-size:1.5rem;">&#8369;<?php echo e(number_format($stats['refunded'],0)); ?></div>
...
<form method="GET" action="<?php echo e(route('admin.payments.index')); ?>">
  <div class="filters-bar">
    <input type="text" name="search" placeholder="Search by ref no or passenger…" value="<?php echo e(request('search')); ?>">
    <select name="status">
    ...
    <?php if(request()->hasAny(['search','status'])): ?>
      <a href="<?php echo e(route('admin.payments.index')); ?>" class="clear-link">Clear</a>
    <?php endif; ?>
  </div>
</form>
...
<div class="table-wrap">
  <table>
    ...
    <tbody>
      ...
      <tr>
        <td><span class="ref-badge"><?php echo e($payment->booking->reference_no ?? '—'); ?></span></td>
        <td>
          <div class="user-chip">
            <div class="user-av"><?php echo e(strtoupper(substr($payment->booking->user->name ?? 'U', 0, 2))); ?></div>
            <div>
              <div style="font-weight:600;color:var(--brown);font-size:.83rem;"><?php echo e($payment->booking->user->name ?? '—'); ?></div>
              <div style="font-size:.71rem;color:rgba(59,42,26,.35);"><?php echo e($payment->booking->user->email ?? ''); ?></div>
            </div>
          </div>
        </td>
        ...
        <td style="font-family:monospace;font-weight:700;color:var(--gold);">&#8369;<?php echo e(number_format($payment->amount,2)); ?></td>
        <td>
          <?php $ps = $payment->status; ?>
          <span class="pill <?php echo e($ps==='paid'?'pill-green':($ps==='pending'?'pill-amber':($ps==='refunded'?'pill-blue':'pill-red'))); ?>">
            <?php echo e(ucfirst($ps)); ?>

          </span>
        </td>
        <td style="font-family:monospace;font-size:.72rem;color:rgba(59,42,26,.35);"><?php echo e($payment->transaction_ref ?? '—'); ?></td>
        <td style="font-size:.75rem;color:rgba(59,42,26,.35);"><?php echo e($payment->created_at->format('M d, Y')); ?></td>
        <td>
          <div style="display:flex;gap:5px;align-items:center;flex-wrap:wrap;">
            ...
            <?php endif; ?>
            <form method="POST" action="<?php echo e(route('admin.payments.status',$payment)); ?>" style="display:inline">
              <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
              <select name="status" class="status-mini" onchange="this.form.submit()">
                <?php $__currentLoopData = ['pending','paid','failed','refunded']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $opt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  <option value="<?php echo e($opt); ?>" <?php echo e($payment->status===$opt?'selected':''); ?>><?php echo e(ucfirst($opt)); ?></option>
  ...
  <div class="pag-wrap">
    <span class="pag-info">Showing <?php echo e($payments->firstItem()); ?>–<?php echo e($payments->lastItem()); ?> of <?php echo e($payments->total()); ?> payments</span>
    <?php echo e($payments->links()); ?>

  </div>
</div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\AMVI User\OneDrive - Agata Mining Ventures\Desktop\laravel\otrs\resources\views/admin/payments/index.blade.php ENDPATH**/ ?>