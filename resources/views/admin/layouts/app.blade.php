...
<head>
    ...
    <title>OTRS Admin — @yield('page-title', 'Admin Panel')</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sb-nav">
        ...
        <a href="{{ route('admin.dashboard') }}" class="sb-item {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}" onclick="closeSidebar()">
            <svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>
            Dashboard
        </a>
        <div class="sb-section">Management</div>
        <a href="{{ route('admin.bookings.index') }}" class="sb-item {{ request()->routeIs('admin.bookings.*') ? 'active' : '' }}" onclick="closeSidebar()">
            <svg viewBox="0 0 24 24"><path d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01" stroke-linecap="round"/></svg>
            Bookings
        </a>
        <a href="{{ route('admin.book-trip') }}" class="sb-item {{ request()->routeIs('admin.book-trip*') ? 'active' : '' }}" onclick="closeSidebar()">
        ...
        <a href="{{ route('admin.users.index') }}" class="sb-item {{ request()->routeIs('admin.users.*') ? 'active' : '' }}" onclick="closeSidebar()">
            <svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>
            Users
        </a>
        <a href="{{ route('admin.trips.index') }}" class="sb-item {{ request()->routeIs('admin.trips.*') ? 'active' : '' }}" onclick="closeSidebar()">
            <svg viewBox="0 0 24 24"><path d="M17.8 19.2L16 11l3.5-3.5C21 6 21 4 19 4c-1 0-1.5.5-3.5 2.5L11 8.2 2.8 6.4 2 7.2l3 5.5-2 1.7v2.2l3-1.5 1.5 3h2.2l1.7-2 5.5 3z"/></svg>
            Trips
        </a>
        <a href="{{ route('admin.schedules.index') }}" class="sb-item {{ request()->routeIs('admin.schedules.*') ? 'active' : '' }}" onclick="closeSidebar()">
            <svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
            Schedules
        </a>
        <div class="sb-section">Finance</div>
        <a href="{{ route('admin.payments.index') }}" class="sb-item {{ request()->routeIs('admin.payments.*') ? 'active' : '' }}" onclick="closeSidebar()">
            <svg viewBox="0 0 24 24"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M2 10h20"/></svg>
            Payments
        </a>
        <div class="sb-section">Promotions</div>
        <a href="{{ route('admin.promos.index') }}" class="sb-item {{ request()->routeIs('admin.promos.*') ? 'active' : '' }}" onclick="closeSidebar()">
