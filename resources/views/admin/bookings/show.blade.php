  ...
  <div>
    ...
    <div class="card">
      ...
      <div class="field-row">
        ...
        <span class="ref-badge">{{ $booking->reference_no }}</span>
      </div>
      <div class="field-row">
        <span class="field-label"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12,6 12,12 16,14"/></svg>Status</span>
        <span class="pill {{ $s==='confirmed'?'pill-green':($s==='pending'?'pill-amber':($s==='cancelled'||$s==='rejected'?'pill-red':($s==='ticketed'?'pill-blue':'pill-gray'))) }}">{{ ucfirst($s) }}</span>
      </div>
      <div class="field-row">
        ...
        <span class="field-value">{{ $booking->passenger_count }} pax</span>
      </div>
      <div class="field-row">
        ...
        <span class="field-value" style="color:var(--gold);font-weight:700;font-family:monospace;">&#8369;{{ number_format($booking->total_amount,2) }}</span>
      </div>
      <div class="field-row">
        ...
        <span class="field-value">{{ $booking->created_at->format('M d, Y h:i A') }}</span>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="field-row">
        ...
        <span class="field-value">{{ $booking->schedule->trip->name }}</span>
      </div>
      <div class="field-row">
        <span class="field-label"><svg viewBox="0 0 24 24"><polyline points="9,18 15,12 9,6"/></svg>Route</span>
        <span class="field-value">{{ $booking->schedule->trip->origin }} → {{ $booking->schedule->trip->destination }}</span>
      </div>
      <div class="field-row">
        ...
        <span class="field-value">{{ ucfirst($booking->schedule->trip->type ?? '—') }}</span>
      </div>
      <div class="field-row">
        ...
        <span class="field-value">{{ $booking->schedule->trip->operator ?? '—' }}</span>
      </div>
      <div class="field-row">
        <span class="field-label"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12,6 12,12 16,14"/></svg>Departure</span>
        <span class="field-value">{{ \Carbon\Carbon::parse($booking->schedule->departure_at)->format('M d, Y h:i A') }}</span>
      </div>
      @else
    ...
    <div class="card">
      ...
      <div class="field-row">
        ...
        <span class="field-value">{{ ucfirst($booking->payment->method ?? '—') }}</span>
      </div>
      <div class="field-row">
        ...
        <span class="field-value" style="color:var(--teal);font-family:monospace;font-weight:700;">{{ number_format($booking->payment->amount,2) }}</span>
      </div>
      <div class="field-row">
        ...
        @php $ps = $booking->payment->status; @endphp
        <span class="pill {{ $ps==='paid'?'pill-green':($ps==='pending'?'pill-amber':'pill-red') }}">{{ ucfirst($ps) }}</span>
      </div>
      <div class="field-row">
        ...
        <span class="field-value" style="font-family:monospace;font-size:.76rem;">{{ $booking->payment->transaction_ref ?? '—' }}</span>
      </div>
      <div class="field-row">
        ...
        <span class="field-value">{{ $booking->payment->paid_at ? $booking->payment->paid_at->format('M d, Y h:i A') : '—' }}</span>
      </div>
      @else
  ...
  <div>
    ...
    <div class="card">
      ...
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;padding-bottom:14px;border-bottom:1px solid rgba(59,42,26,.07);">
        <div class="user-av-lg">{{ strtoupper(substr($booking->user->name,0,2)) }}</div>
        <div>
          <div style="font-weight:700;color:var(--brown);font-size:.95rem;">{{ $booking->user->name }}</div>
          <div style="color:rgba(59,42,26,.38);font-size:.75rem;margin-top:2px;">{{ $booking->user->email }}</div>
        </div>
      </div>
      <div class="field-row">
        ...
        <span class="field-value">{{ $booking->user->phone ?? '—' }}</span>
      </div>
      <div class="field-row">
        ...
        <span class="field-value">{{ ucfirst($booking->user->role) }}</span>
      </div>
      <div class="field-row">
        ...
        <span class="pill {{ $booking->user->status==='active'?'pill-green':'pill-red' }}">{{ ucfirst($booking->user->status ?? 'active') }}</span>
      </div>
      @else
    ...
    <div class="card">
      ...
      @elseif($s === 'cancelled' || $s === 'rejected')
        <div class="state-banner banner-rejected">
          <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
          <span class="state-banner-text">This booking has been {{ $s }}.</span>
        </div>
      ...
      @if(!in_array($s, ['confirmed','cancelled','rejected']))
          ...
          <form method="POST" action="{{ route('admin.bookings.status',$booking) }}">
            ...
            <div class="status-form">
              <select name="status" class="status-select">
                @foreach(['pending','confirmed','cancelled','expired','ticketed'] as $opt)
                  <option value="{{ $opt }}" {{ $booking->status===$opt?'selected':'' }}>{{ ucfirst($opt) }}</option>
                @endforeach
              </select>
