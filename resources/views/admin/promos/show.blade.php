...
@section('page-title', 'Promo Detail')
...
<div class="page-header">
  <div>
    <div class="page-title">{{ $promo->title }}</div>
    <div class="page-sub">Promo ID #{{ $promo->id }}</div>
  </div>
...
<div class="hero-card">
  <div>
    <div class="hero-discount">{{ $promo->formatted_discount }}</div>
    <div class="hero-sub">{{ $promo->discount_type === 'percentage' ? 'Percentage discount' : 'Fixed amount off' }}</div>
  </div>
  <div style="text-align:center;">
    <div class="hero-code-label">Promo Code</div>
    <div class="code-tag">{{ $promo->promo_code }}</div>
  </div>
  <div style="text-align:right;">
    @php $status = $promo->status; @endphp
    <span class="pill {{ $status==='active'?'pill-green':($status==='upcoming'?'pill-amber':'pill-gray') }}">
      {{ ucfirst($status) }}
    </span>
    <div class="hero-validity">{{ $promo->start_date->format('M d, Y') }} — {{ $promo->end_date->format('M d, Y') }}</div>
  </div>
</div>
...
<div class="card">
  ...
  <div class="detail-grid">
    <div class="detail-item">
      <label>Title</label>
      <div class="val">{{ $promo->title }}</div>
    </div>
    <div class="detail-item">
      <label>Discount Type</label>
      <div class="val">{{ $promo->discount_type === 'percentage' ? 'Percentage (%)' : 'Fixed Amount (₱)' }}</div>
    </div>
    <div class="detail-item">
      <label>Discount Value</label>
      <div class="val" style="color:var(--gold);font-weight:700;">{{ $promo->formatted_discount }}</div>
    </div>
    <div class="detail-item">
      <label>Start Date</label>
      <div class="val">{{ $promo->start_date->format('F d, Y') }}</div>
    </div>
    <div class="detail-item">
      <label>End Date</label>
      <div class="val">{{ $promo->end_date->format('F d, Y') }}</div>
    </div>
    <div class="detail-item">
      <label>Created At</label>
      <div class="val">{{ $promo->created_at->format('M d, Y') }}</div>
    </div>
  </div>
  ...
  <div style="margin-top:18px;padding-top:16px;border-top:1.5px solid rgba(59,42,26,.07);">
    ...
    <p style="font-size:.85rem;color:rgba(59,42,26,.55);line-height:1.65;">{{ $promo->description }}</p>
  </div>
  @endif
</div>
  ...
  @elseif($promo->trips->count())
    <div>
      ...
      <span class="trip-pill">
        ...
        <span class="t-route">{{ $trip->origin }} → {{ $trip->destination }}</span>
      </span>
      @endforeach
    </div>
  @else
