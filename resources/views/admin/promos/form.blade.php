{{--
  ...
  Expects: $promo (Promo|null), $trips (Collection), $action (string), $method ('POST'|'PUT')
--}}
<style>
  ...
  <div class="form-card">
    ...
    <div class="form-group">
      ...
      <textarea name="description" class="f-input"
                placeholder="Optional note about this promo…">{{ old('description', $promo->description ?? '') }}</textarea>
    ...
    <div class="form-group">
      ...
      @error('promo_code')<div class="f-error">{{ $message }}</div>@enderror
    </div>
  </div>
  ...
  <div class="form-card">
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        @error('discount_type')<div class="f-error">{{ $message }}</div>@enderror
      </div>
      <div class="form-group">
        ...
        @error('discount_value')<div class="f-error">{{ $message }}</div>@enderror
      </div>
    </div>
  </div>
  ...
  <div class="form-card">
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        @error('start_date')<div class="f-error">{{ $message }}</div>@enderror
      </div>
      <div class="form-group">
        ...
        @error('end_date')<div class="f-error">{{ $message }}</div>@enderror
      </div>
    </div>
    ...
    <div class="toggle-wrap">
      <label class="toggle">
        <input type="checkbox" id="applies_to_all" name="applies_to_all" value="1"
               {{ old('applies_to_all', ($promo->applies_to_all ?? true) ? '1' : '') == '1' ? 'checked' : '' }}
               onchange="toggleTripSelector()">
        <span class="slider"></span>
      </label>
      <span class="toggle-text">Apply to <strong>all trips</strong></span>
    </div>
    <div id="trip-selector" style="{{ old('applies_to_all', ($promo->applies_to_all ?? true) ? '1' : '') == '1' ? 'display:none' : '' }}">
      <label class="f-label" style="margin-bottom:8px;">Select Trips</label>
      @php $selectedIds = old('trip_ids', isset($promo) ? $promo->trips->pluck('id')->toArray() : []); @endphp
      @if($trips->count())
        <div class="trip-grid">
          @foreach($trips as $trip)
          <label class="trip-check">
            <input type="checkbox" name="trip_ids[]" value="{{ $trip->id }}"
                   {{ in_array($trip->id, (array)$selectedIds) ? 'checked' : '' }}>
      ...
      @endif
      @error('trip_ids')<div class="f-error">{{ $message }}</div>@enderror
    </div>
  </div>

  <div style="display:flex;gap:10px;align-items:center;">
    <button type="submit" class="btn-save">{{ $method === 'PUT' ? 'Save Changes' : 'Create Promo' }}</button>
    <a href="{{ route('admin.promos.index') }}" class="btn-cancel">Cancel</a>
  </div>
</form>
