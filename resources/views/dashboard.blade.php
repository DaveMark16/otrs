...
<div class="main">
  <div class="topbar">
    <div class="topbar-title">Welcome back, <em style="font-style:italic;color:var(--teal);">{{ explode(' ', auth()->user()->name)[0] }}</em></div>
  ...
  <div class="content">
    ...
    <div class="greeting">
      ...
      <h1>Hello, <em>{{ explode(' ', auth()->user()->name)[0] }}</em> 👋</h1>
      <div class="greeting-sub">Here's an overview of your journeys and upcoming adventures.</div>
    </div>

    @php $activePromos = \App\Models\Promo::active()->orderBy('end_date')->limit(4)->get(); @endphp
    @if($activePromos->isNotEmpty())
    <div class="promo-banner">
      <div class="promo-banner-icon">🏷️</div>
      <div class="promo-banner-body">
        <div class="promo-banner-label">{{ $activePromos->count() === 1 ? 'Active Promo' : $activePromos->count() . ' Active Promos' }} — save on your next booking</div>
        <div class="promo-banner-codes">
          @foreach($activePromos as $ap)
            <span class="pbc" title="{{ $ap->title }} · {{ $ap->formatted_discount }} off"
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card brown">
        ...
        <div class="stat-value" style="font-size:1.5rem;">₱{{ number_format($totalSpent, 0) }}</div>
      ...
      <div class="panel">
        ...
        <div class="panel-body">
          @forelse($recentBookings as $booking)
            @php $sched = $booking->schedule; $trip = $sched?->trip; @endphp
            <div class="booking-item">
              <div class="booking-route-wrap">
                <div class="booking-plane-icon">✈</div>
                <div>
                  <div class="booking-route">{{ $trip->origin_country ?? ($trip->origin ?? '?') }} → {{ $trip->destination_country ?? ($trip->destination ?? '?') }}</div>
    ...
    <div class="panel">
      ...
      <div class="panel-body">
        @forelse($upcomingTrips as $trip)
          @php $sc = $trip->schedule; $tr = $sc?->trip; @endphp
          <div class="upcoming-item">
            <div class="booking-route-wrap">
              <div class="booking-plane-icon">✈</div>
              <div>
                <div class="upcoming-route">{{ $tr->origin ?? '?' }} <span style="color:rgba(59,42,26,.35);font-weight:400;">→</span> {{ $tr->destination ?? '?' }}</div>
                @if($tr->origin_country ?? null)
                  <div class="upcoming-country">{{ $tr->origin_country }} → {{ $tr->destination_country ?? '' }}</div>
                @endif
                <div class="upcoming-dep">{{ $sc?->departure_at?->format('M d, Y · h:i A') ?? '—' }}</div>
              </div>
            </div>
            <span class="upcoming-badge">{{ ucfirst($trip->status) }}</span>
          </div>
        @empty
          <div class="empty">
            <div class="empty-icon">🗺</div>
            No upcoming trips. <a href="{{ route('bookings.create') }}" style="color:var(--teal);font-weight:600;">Book one now →</a>
          </div>
        @endforelse
      </div>
    </div>
